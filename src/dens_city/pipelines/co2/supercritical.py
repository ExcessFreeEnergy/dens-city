...
def compute_supercritical_crossovers(
    ...
    for t_idx, T in enumerate(temperatures):
        ...
        # Fisher-Widom transition: structural crossover where S(k) first develops a peak k_max > 0
        # (crossover from monotonic exponential decay to oscillatory decay)
        fw_idx = best_chi_idx
        for d_idx, rho_b in enumerate(densities):
            r_coords, c2_r = compute_radial_c2(torch_c1_fn, rho_b, T)
            s_k = compute_structure_factor(c2_r, r_coords, rho_b, k_vals)
            # Find if there is an off-zero peak in S(k)
            if len(s_k) > 2 and np.any(s_k[1:] > s_k[0] * 1.05):
                fw_idx = d_idx
                break
        fisher_widom_densities.append(densities[fw_idx])
...
def compute_orientational_density_and_order(
    ...
    # Linear rigid CO2 molecule parameters (TraPPE: L = 2.32 A, sigma_O = 3.03 A, Q = -4.3 D*A)
    ...
    for _ in range(20):
        if coln_model is not None and hasattr(coln_model, "dir_net"):
            with torch.no_grad():
            ...
            # Self-consistent quadrupole mean-field orientational potential
            # Q_CO2 = -4.3 D*A gives planar wall alignment under density gradients
            drho = np.gradient(rho_bar, z_grid)
            c20_profile = -np.sign(drho) * (np.abs(drho) / max(1e-4, rho_bulk))
        ...
        for iz in range(n_z):
            ...
            if not np.any(allowed):
                ...
                weight[allowed] = np.exp(np.clip(c20_profile[iz] * p2[allowed], -25.0, 25.0))
