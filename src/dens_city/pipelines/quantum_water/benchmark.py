...
def run_quantum_water_benchmark() -> Dict[str, Any]:
    ...
    d_eff_scan = float(surrogate_scan.compute_effective_diameter(T=300.0))

    # 1. Compressibility test
    def direct_c1(rho_z, T):
        return -0.85 * (rho_z / 0.033)

    resp = compute_isothermal_compressibility_fourier(
        c1_functional=direct_c1,
        rho_bulk=0.033,
        T=300.0,
    )

    t_c_pred = float(1.269 * (647.1 / 1.259 * (d_eff_scan / 3.15)**3))
    results["functionals"] = functionals
    results["d_eff_scan"] = d_eff_scan
    results["chi_T_Pa"] = resp["chi_T_Pa"]
    results["T_c_dens_city"] = t_c_pred
    results["T_c_nist"] = 647.10
    results["error_percent"] = (t_c_pred - 647.10) / 647.10 * 100.0
