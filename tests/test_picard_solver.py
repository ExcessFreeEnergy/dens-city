...
from dens_city.solver.thermo_integration import compute_bulk_pressure, compute_excess_free_energy


def test_picard_solver_convergence():
    # Simple hard-sphere-like local functional
    c1_fn = lambda rho, T: -0.8 * (rho / 0.033)
    ...
    rho, converged, it, res = solver.solve(
        z_coords, v_ext, T=300.0, mu=-3000.0 * 1.380649e-23, rho_bulk=0.033
    )
...
def test_thermodynamic_integration():
    c1_fn = lambda rho, T: -0.5 * (rho / 0.033)
